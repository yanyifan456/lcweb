...
export default {
    ...
    methods: {
        ...
        handleCheck(currentNode, checkedInfo) {
            if (this.isDisabled) return

            const tree = this.$refs.tree
            if (!tree) return

            const rootIds = this.data.map(n => n.id)

            // 当前选中的所有 key
            let checkedKeys = checkedInfo.checkedKeys

            // 找出被选中的一级节点
            const checkedRoots = checkedKeys.filter(k => rootIds.includes(k))

            // 如果一级节点 > 1，只保留最后点击的
            if (checkedRoots.length > 1) {
                checkedKeys = checkedKeys.filter(k => !rootIds.includes(k))
                checkedKeys.push(currentNode.id)
            ...
            const orgIds = checkedNodes.map(n => n.orgId)
