<template>
    <div class="boxbig" ref="container">
        ...
        <el-table :data="tableData" border style="width: 100%;border-radius: 20px 20px 0px 0px;margin-top: 30px;"
            ...
            <el-table-column prop="" v-if="userauth && (userauth.includes('superAdmin') || userauth.includes('admin'))"
                label="医嘱确认" align="center">
                <template slot-scope="scope">
            ...
            <el-table-column v-if="userauth && (userauth.includes('superAdmin') || userauth.includes('admin'))"
                class="aasd" fixed="right" style="margin-top: 0;" label=" 操作" width="250" align="center">
                <template slot-scope="scope">
                    <el-button type="text" size="small" class="action-btn detail-btn"
...
export default {
    ...
    methods: {
        ...
        dayinyizhuzxd(row) {
            if (row.confirmAdvice == 0) {
                this.$confirm("请先确认医嘱！", "提示", {
            ...
            this.ekk = row;
            this.asdsq = JSON.parse(this.ekk.nurseAdvice);
