<template>
  <div class="boxbig" ref="container">
    ...
    <div class="box-c">
      ...
      <div class="filter-row">
        <div class="filter-item">
          ...
          <el-input v-model="deviceName" placeholder="请输入" class="filter-input"></el-input>
        </div>
        <div class="filter-item">
          <span class="filter-label" style="margin-left: 27px">状态：</span>
          <el-select v-model="deviceFlag" placeholder="请选择">
            <el-option v-for="item in deviceFlagList" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </div>
    ...
    <el-table :data="tableData" border style="width: 100%;border-radius: 20px 20px 0px 0px;margin-top: 30px;"
      ...
      <el-table-column prop="deviceFlag" label="状态" width="110" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.deviceFlag == "1" ? "启用" : "未启用" }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="testItem" label="检测项目" align="center">
        <template slot-scope="scope">
          <span v-for="item in scope.row.matchedItems" :key="item.itemId">
            {{ item.itemName }}
          </span>
        </template>
      </el-table-column>
...
<script>
import { getDeviceManage, deleteDeviceManage } from "@/api/deviceManage";
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push("/CreateEquipment");
    },
    handleEdit(row) {
    ...
    handleDelete(row) {
      ...
      this.deviceBrand = row.deviceBrand
    ...
    async onDelConfirm() {
      ...
      try {
        ...
        const res = await deleteDeviceManage({
    ...
    async getData(params) {
      try {
        const res = await getDeviceManage(params);
        // 添加id列
